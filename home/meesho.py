...
def extract_product_info(block_text):
    ...
    try:
        ...
        if len(flat_data) >= 5:
            ...
            if "Free" in flat_data and "Size" in flat_data:
                try:
            ...
            if not product_info["sku"]:
                ...
                for i, item in enumerate(flat_data):
                    if re.match(qty_pattern, item) and len(item) <= 3:  # Qty is usually 1-3 digits
                        if i >= 2 and i + 2 < len(flat_data):
                            product_info["sku"] = " ".join(flat_data[:i-1])
                ...
                if not product_info["sku"] and len(flat_data) >= 5:
                    product_info["sku"] = " ".join(flat_data[:-4])
                    product_info["size"] = flat_data[-4]
                    product_info["qty"] = flat_data[-3]
                    product_info["color"] = flat_data[-2]
                    product_info["order_no"] = flat_data[-1]
